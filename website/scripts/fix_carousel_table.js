...
async function fixCarouselTableColumns() {
  try {
    ...
    // Check if button_text and button_link columns exist and cta_text, cta_link don't exist
    ...
    const buttonColumnsExist = columnCheck.data[0].button_columns_count > 0;
    ...
    if (buttonColumnsExist) {
      ...
      const { data: ctaCheck, error: ctaError } = await supabase.rpc('exec', { 
        query: `
          SELECT 
      ...
      const ctaColumnsExist = ctaCheck.data[0].cta_columns_count > 0;
