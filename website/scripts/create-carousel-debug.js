 ...
 * 
 * This script helps debug carousel image display issues by adding a special debug mode
 * to the carousel that shows detailed information about image loading and display.
 */

// Create a debug.js file that can be loaded in the browser console
const fs = require('fs');
...
try {
  fs.writeFileSync(path.join(process.cwd(), 'public', 'carousel-debug.js'), DEBUG_SCRIPT);
  console.log('✅ Carousel debug script created at: /public/carousel-debug.js');
  console.log('👉 To use, open your browser console and execute:');
  console.log('   fetch("/carousel-debug.js").then(r => r.text()).then(code => eval(code))');
} catch (error) {
