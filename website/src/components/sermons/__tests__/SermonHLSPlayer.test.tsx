...
describe('SermonPlayer Component with HLS', () => {
  ...
  test('renders HLS video player when videoUrl is provided with m3u8 extension', () => {
    render(<SermonPlayer {...mockSermonProps} />);
  ...
  test('initializes HLS.js when provided with m3u8 URL', () => {
    ...
    // HLS.js should be initialized and instance methods should be called
    expect(HlsMock).toHaveBeenCalled();
    const mockInstance = HlsMock.mock.results[0]?.value;
  ...
  test('falls back to native HLS in Safari when HLS.js is not supported', () => {
    ...
    const canPlayTypeSpy = jest.fn().mockReturnValue('maybe' as CanPlayTypeResult);
    ...
    // Verify video source was set for native playback
    expect(videoElement).toHaveAttribute('src', mockSermonProps.videoUrl);
