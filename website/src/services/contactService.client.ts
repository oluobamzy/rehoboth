// src/services/contactService.client.ts
// Client-side version of ContactService for admin dashboard
'use client';
...
export class ContactServiceClient {
  ...
  static async getAllMessages(): Promise<ContactMessage[]> {
    try {
      console.log('Client: Fetching all messages...');
      
      // Check if user is authenticated
      const { data: { user } } = await supabase.auth.getUser();
      console.log('Client: Current user:', user?.email || 'Not authenticated');
